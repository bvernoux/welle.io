 ...
 *
*/
#ifndef __HYDRASDR_RADIO__
#define __HYDRASDR_RADIO__
...
enum class CHydrasdr_IOCTL {
    SET_BIAS_TEE
};

class CHydrasdr : public CVirtualInput {
public:
    CHydrasdr(RadioControllerInterface& radioController);
    ~CHydrasdr(void);
    CHydrasdr(const CHydrasdr&) = delete;
    CHydrasdr& operator=(const CHydrasdr&) = delete;
...
private:
    ...
    const int HYDRASDR_SAMPLERATE = 4096000;
    const int HYDRASDR_GAIN_MIN = 0;
    const int HYDRASDR_GAIN_MAX = 21;
    ...
    static int callback(hydrasdr_transfer_t*);
